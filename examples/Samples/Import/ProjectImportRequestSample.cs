...
{
    public class ProjectImportRequestSample
    {
        ...
        {
            DateTime start = DateTime.Now.AddDays(i + 2);
            DateTime end = start.AddSeconds(86400);

            string TaskNameGenerator(int y) => $"Task {y}";
            string taskName = TaskNameGenerator(i);
            string previousTaskName = TaskNameGenerator(i - 1);
            ...
            Task taskRequest = new Faker<Task>()
                ...
                .RuleFor(x => x.TaskNo, f => taskName)
                .RuleFor(x => x.PredecessorTaskNo, f => previousTaskName)
                .RuleFor(x => x.ShortDescription, f => $"Project task {i}")
                .RuleFor(x => x.Description, f => $"Project {jobRequest.JobNo} task {i}")
                .RuleFor(x => x.TaskType, f => TaskType.Project)
                .RuleFor(x => x.StartDate, f => start)
                .RuleFor(x => x.EndDate, f => end)
                .RuleFor(x => x.RequiredTotalDurationInSeconds, f => 86400)
                .RuleFor(x => x.DurationInSeconds, f => 86400)
                .RuleFor(x => x.PlanningUOMConversion, f => 3600)
                .RuleFor(x => x.PlanningUOM, f => "S")
        ...
        {
            string projectName = new ProjectNameList().ToString();

            Job jobRequest = new Faker<Job>()
                .RuleFor(x => x.SourceApp, f => "IMPORT")
                .RuleFor(x => x.SourceType, f => "IMPORT")
                .RuleFor(x => x.JobNo, f => projectName)
                .RuleFor(x => x.Name, f => projectName)
                .RuleFor(x => x.Type, f => "1")
                .RuleFor(x => x.AllowDependencies, f => true)
                .RuleFor(x => x.AvailableInGantt, f => true)
